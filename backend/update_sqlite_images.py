...
db_path = r"c:\Users\malle\OneDrive\Desktop\cafe saas appliction\backend\teatime.db"
...
if os.path.exists(db_path):
    ...
    for row_id, name in rows:
        ...
        matched = None
        for k, v in DISH_IMAGE_MAPPING.items():
            if k in name_l or name_l in k:
                matched = v
                break
        if matched:
            cur.execute("UPDATE menu_items SET image_url = ? WHERE id = ?", (matched, row_id))
            updated += 1
            print(f"[{row_id}] Updated {name} -> {matched}")
    conn.commit()
    conn.close()
    print(f"\nUpdated {updated} items in SQLite DB.")
else:
    print("DB file not found, will update seed.py.")
